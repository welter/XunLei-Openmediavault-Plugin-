 ...
 */
if ( version_compare( PHP_VERSION, XunLeiRPC::MIN_PHPVER, '<' ) )
  die( "The XunLeiRPC class requires PHP version {XunLeiRPC::XUNLEIRPC_MIN_PHPVER} or above." . PHP_EOL );
...
class XunLeiRPC
{
   ...
   */
  const HTTP_UA = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/43.0.2357.81 Chrome/43.0.2357.81 Safari/537.36';
  ...
  public function openVipChannel($id)
   {
   	$args=array(
   			"v"=>2,
   			"taskid"=>$id,
   	...
   	return $this->request("openVipChannel",$args);	  
   }
   public function openLixianChannel($id)
   {
   	  $args=array(
   	  		"v"=>2,
   	  		"taskid"=>$id,
   	  ...
   	  return $this->request("openLixianChannel",$args);
   }
  ...
  /**
   * Get job information in xunlei, if the ids parameter is
   * empty all jobs will be returned. 
   *
   * @param int kclass type of job
   * @param int|array ids A list of transmission torrent ids
   */
  public function get ( $ids=array() ,$kclass=0,$starting = 0, $count = 999999, $abs_path = True, $fixed_id = false )
  {
    if ( !is_array( $ids ) ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
    $args = array(
      "v" => 2,
      "type" => intval($kclass),
    	...
    	"number"=>$count,
    	"needUrl"=>intval(True),
    	"abs_path"=>intval($abs_path),
    	"fix_id"=>intval($fixed_id)
    );
    $r=$this->request( "list", $args );
    var_dump($r);
    $tasks=array();
    if (!empty($ids)){
    foreach($r->tasks as $ri)
    {
    	echo $ri->id."\n";
    	if (in_array($ri->id,$ids)) 
    	{
    		$tasks[]=$ri; 
    	}
    }
...
//    if ( !is_array( $arguments ) )
 //     throw new XunLeiRPCException( 'Arguments must be given as array', XunLeiRPCException::E_INVALIDARG );
...
//    if( !$this->session_id )
 ...
 //       throw new XunLeiRPCException( 'Unable to acquire X-Ware-Session-Id', XunLeiRPCRPCException::E_SESSIONID );
    //Build http
    $url=$this->url;
    ...
    if( $this->debug ) echo "XUNLEIRPC_DEBUG:: request( method=$method, ...):: Stream context created with options:" .
    ...
    var_dump($context);
    if ( $fp = fopen( $url, 'rb' , false, $context) ) {	// Open a filepointer to the data, and use fgets to get the result
      $response = '';
    //  while( $row = fgets( $fp ) ) {
        //$response.= trim( $row )."\n";
      //}
      $response=fgets($fp);
      if( $this->debug ) echo "XUNLEIRPC_DEBUG:: request( method=$method, ...):: POST Result: ".
                            PHP_EOL . print_r( $response, true );
    } else
      throw new XunLeiRPCException( 'Unable to connect to '.$this->url, XunLeiRPCException::E_CONNECTION );

    // Check the response (headers etc)
    $stream_meta = stream_get_meta_data( $fp );
    fclose( $fp );
    if( $this->debug ) echo "XUNLEIRPC_DEBUG:: request( method={$method}, ...):: Stream meta info: ".
                            PHP_EOL . print_r( $stream_meta, true );
    if( $stream_meta['timed_out'] )
      throw new XunLeiRPCException( "Timed out connecting to {$this->url}", XunLeiRPCException::E_CONNECTION );
    if( substr( $stream_meta['wrapper_data'][0], 9, 3 ) == "401" )
      throw new XunLeiRPCException( "Invalid username/password.", XunLeiRPCException::E_AUTHENTICATION );
    elseif( substr( $stream_meta['wrapper_data'][0], 9, 3 ) == "409" )
      throw new XunLeiRPCException( "Invalid X-Transmission-Session-Id. Please try again after calling GetSessionID().", XunLeiRPCException::E_SESSIONID );
    ...
    switch ($format) {
        case 'json':
          ...
          if ($r === null) {
            throw new XunLeiRPCException("failed to decode $response as json", XunLeiRPCException::E_SESSIONID );
          }
          break;
        case 'xml':
 ...
 //         var_dump($r);
          $rtn = (array)$r->rtn;
          if (rtn<>0) throw new XunLeiRPCException("rtn is not 0, but $rtn", XunLeiRPCException::E_SESSIONID );
          if ($verb==="POST"){
          if (array_key_exists('tasks',$r)) {
            foreach ($r->tasks as $taskResponse) {
                if ($rtn <>0) throw new XunLeiRPCException("rtn is not 0, but $rtn", XunLeiRPCException::E_SESSIONID );
            }
          }
        } else if ($verb==="GET"){
...
//    var_dump($r); 
  ...
  /**
   * Performs an empty GET on the XunLei RPC to get the X-Ware-Session-Id
  ...
  {
    if( !$this->url )
      throw new XunLeiRPCException( "Class must be initialized before GetSessionID() can be called.", XunLeiRPCException::E_INVALIDARG );
    ...
    if( $this->debug ) echo "XUNLEIRPC_DEBUG:: GetSessionID():: Stream context created with options:".
                            PHP_EOL . print_r( $contextopts, true );
    ...
    $stream_meta = stream_get_meta_data( $fp );
    fclose( $fp );
    if( $this->debug ) echo "XUNLEIRPC_DEBUG:: GetSessionID():: Stream meta info: ".
                            PHP_EOL . print_r( $stream_meta, true );
    if( $stream_meta['timed_out'] )
      throw new XunLeiRPCException( "Timed out connecting to {$this->url}", XunLeiRPCException::E_CONNECTION );
    if( substr( $stream_meta['wrapper_data'][0], 9, 3 ) == "401" )
    ...
    elseif( substr( $stream_meta['wrapper_data'][0], 9, 3 ) == "409" )	// This is what we're hoping to find
    {
      ...
      {
        if( strpos( $header, 'X-Ware-Id: ' ) === 0 )
        {
          if( $this->debug ) echo "XUNLEIRPC_DEBUG:: GetSessionID():: Session-Id header: ".
      ...
      if( ! $this->session_id ) {	// Didn't find a session_id
        throw new XunLeiRPCException( "Unable to retrieve X-Ware-Session-Id", XunLeiRPCException::E_SESSIONID );
      }
    } else {
      throw new XunLeiRPCException( "Unexpected response from XunLei RPC: ".$stream_meta['wrapper_data'][0] );
    }
   ...
   */
  public function __construct( $url = 'http://localhost:9000', $username = null, $password = null, $return_as_array = false )
  {
    // server URL
    $this->url = $url;
 ...
 */
class XunLeiRPCException extends Exception
{
  /**
